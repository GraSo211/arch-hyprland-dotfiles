...
Process {
    ...
    stdout: SplitParser {
        onRead: line => {
            getDefaultKeybinds.buffer += line
        }
    }

    onFinished: {
        try {
            root.defaultKeybinds = JSON.parse(buffer)
            console.log("Parsed default keybinds OK")
        } catch (e) {
            console.error("Failed parsing default keybinds:", buffer, e)
        }
        buffer = ""
    }
}
